...
fig.suptitle('Performance of Former Guess Generation Method')
...
gs = gridspec.GridSpec(3, 6, height_ratios=[1, 1, 0.1], width_ratios=[2/3, 2/3, 2/3, 2/3, 2/3, 2/3])
ax_error = fig.add_subplot(gs[0, 0:3])
ax_ratio = fig.add_subplot(gs[0, 3:6])
...
ax_error.legend(by_label.values(), by_label.keys(), markerscale=2, fontsize=8, loc='upper left')

fig.colorbar(plt.cm.ScalarMappable(norm=LogNorm(min_var, max_var), cmap=cmap),
             cax=ax_color, orientation='horizontal', aspect=25,
             label=r'Perturbation Magnitude of Initial Costates, $m$')
