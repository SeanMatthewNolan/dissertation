...
fig1 = plt.figure(figsize=(3.5, 2.75))

gs = gridspec.GridSpec(2, 2)

ax_h   = fig1.add_subplot(gs[0, 0])
ax_phi = fig1.add_subplot(gs[0, 1])
ax_alpha = fig1.add_subplot(gs[1, 0])
ax_beta  = fig1.add_subplot(gs[1, 1])

fig2 = plt.figure(figsize=(5.5, 2.25))

gs2 = gridspec.GridSpec(2, 3)
ax_lam_h   = fig2.add_subplot(gs2[0])
ax_lam_phi = fig2.add_subplot(gs2[1])
ax_lam_the = fig2.add_subplot(gs2[2])
ax_lam_v   = fig2.add_subplot(gs2[3])
ax_lam_gam = fig2.add_subplot(gs2[4])
ax_lam_psi = fig2.add_subplot(gs2[5])

linewidth = 2

ax_h.plot(np.rad2deg(guess.x[1, :]), guess.x[0, :] / 1e5, label='Guess',    linewidth=linewidth)
ax_h.plot(np.rad2deg(sol.x[1, :]),   sol.x[0, :] / 1e5,   label='Solution', linewidth=linewidth)
ax_h.set_xlabel(r'Downrange $\phi$, [deg]')
ax_h.set_ylabel('Altitude $h$,\n' r'[$\times 10^5$ ft]')

ax_phi.plot(guess.x[3, :] / 1e3, guess.x[0, :] / 1e5, label='Guess',    linewidth=linewidth)
ax_phi.plot(sol.x[3, :] / 1e3,   sol.x[0, :] / 1e5,   label='Solution', linewidth=linewidth)
ax_phi.set_xlabel(r'Velocity $v$, [$\times 10^3$ ft/s]')
ax_phi.set_ylabel('Altitude $h$,\n' r'[$\times 10^5$ ft]')
...
# ax_ham.set_ylabel(r'Ham. $H$, [rad]')
# # ax_ham.set_ylim((-90, 90))

handles, labels = ax_h.get_legend_handles_labels()
fig1.legend(handles, labels, ncols=2, loc='lower center')

handles, labels = ax_lam_h.get_legend_handles_labels()
fig2.legend(handles, labels, ncols=2, loc='lower center')
...
guess_nu = np.concatenate((guess.nu0, guess.nuf))
sol_nu = np.concatenate((sol.nu0, sol.nuf))

guess_nu_str = ''
sol_nu_str = ''
for g_nu, s_nu in zip(guess_nu, sol_nu):
    guess_nu_str += f'& {g_nu:12.4g} '
    sol_nu_str += f'& {s_nu:12.4g} '
