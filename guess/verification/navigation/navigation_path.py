...
from giuseppe import ContinuationHandler, auto_propagate_guess, StrInputProb, SciPySolver, SymDual, Timer,\
    SymPenaltyConstraintHandler, InteractiveGuessGenerator
...
k0 = comp_dual.default_values

steps = 25

for t_prop in [5, 10, 25, 50]:
    data = []
    for xb, yb in itertools.product(np.linspace(0, 500, steps), np.linspace(50, 500, steps)):
        print(f'xb = {xb:6.2f}, yb = {yb:6.2f}')

        k = copy.copy(k0)
        k[1] = xb
        k[2] = yb

        guess = auto_propagate_guess(comp_dual, control=0 / 180 * 3.14159, t_span=t_prop, k=k)
        sol = solver.solve(guess)

        print(f'Converged: {sol.converged}')

        data.append({'xb': xb, 'yb': yb, 'guess': guess, 'sol': sol, 'converged': sol.converged})

    with open(f'nav_path_t{t_prop:d}.data', 'wb') as file:
