...
for t_span in t_spans:
    for alpha in alphas:
        t_spans_out.append(t_span)
        alphas_out.append(alpha)

        guess = auto_propagate_guess(comp_dual, control=(alpha / 180 * 3.14159, 0), t_span=t_span,
                                     quadrature=quad)
        guesses.append(guess)

        try:
            attempt = solver.solve(guess)
            attempts.append(attempt)
            if attempt.converged:
                print(f'Alpha: {alpha:.2g}; Time: {t_span:.2g}; Converged')
                converged.append(True)
            else:
                print(f'Alpha: {alpha:.2g}; Time: {t_span:.2g}; Not Converged')
                converged.append(False)
        except (ZeroDivisionError, ValueError):
            print(f'Alpha: {alpha:.2g}; Time: {t_span:.2g}; Not Converged')
            converged.append(False)

with open(f'giuseppe_guess_test_{quad}.pickle', 'wb') as file:
    pickle.dump((guesses, t_spans_out, alphas_out, converged), file)
