...
fig_ex = plt.figure(figsize=(5.5, 2.))
...
ax_path.plot(sol_none.x[0, :], sol_none.x[1, :], '--', label='Min. Time')
ax_path.plot(xb_path, yb_path, '*', label='Beacon')
ax_path.set_xlabel(r'Position, $x$ [ft]')
ax_path.set_ylabel(r'Position, $y$ [ft]')

fig_ex.legend(loc='lower center', ncols=4, frameon=False)

# ax_theta = fig_ex.add_subplot(gs[0, 1])
# ax_theta.plot(sol_path.t, np.rad2deg(sol_path.x[2, :]), label='Path Cost')
# ax_theta.plot(sol_term.t, np.rad2deg(sol_term.x[2, :]), label='Terminal Cost')
# ax_theta.plot(sol_none.t, np.rad2deg(sol_none.x[2, :]), '--', label='Min Time Straight')
# ax_theta.set_xlabel(r'$t$ [s]')
# ax_theta.set_ylabel(r'Heading Angle,' + '\n' + r'$\theta$ [deg]')

ax_omega = fig_ex.add_subplot(gs[0, 1])
ax_omega.plot(sol_path.t, np.rad2deg(sol_path.u[0, :]), label='Path Cost')
ax_omega.plot(sol_term.t, np.rad2deg(sol_term.u[0, :]), label='Terminal Cost')
ax_omega.plot(sol_none.t, np.rad2deg(sol_none.u[0, :]), '--', label='Min. Time')
ax_omega.set_xlabel(r'$t$ [s]')
ax_omega.set_ylabel(r'Turn Rate,' + '\n' + r'$\omega$ [deg / s]')
...
ax_var_y = fig_ex.add_subplot(gs[0, 2])
...
ax_var_y.plot(sol_none.t, sol_none.x[3, :] + sol_none.x[6, :], '--', label='Min. Time')
ax_var_y.set_xlabel(r'$t$ [s]')
