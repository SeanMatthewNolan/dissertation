...
# bank_sol = bank_set[-1]
sol = heat_rate_set[0]
bank_sol = heat_rate_set[int(len(heat_rate_set)/2)]
...
ax_hv.plot(bank_sol.x[3, :] / 1e3, bank_sol.x[0, :] / 1e5, label=r'$|\beta|$ < 70 deg')
...
ax_gam.plot(bank_sol.t, bank_sol.x[4, :] * 180 / np.pi, label=r'$q < 70 \mathregular{BTU/ft^2/sec}$')
...
ax_heat.set_ylabel(r'Heat-Rate, $q$ [$\mathregular{BTU/ft^2/sec}$]')
