...
from giuseppe.data_classes import remesh
...
cont.add_linear_series(200, {'xi': -90 / 180 * np.pi})
...
vf_min = 5_000
vf_max = 15_000
for xi in np.linspace(np.deg2rad(-170), np.deg2rad(170), 35):
    print(f'{xi * 180 / 3.1415}')

    cont_xi = ContinuationHandler(num_solver, sol_set[-1])
    cont_xi.add_linear_series(21, {'xi': xi})
    sol_set_xi = cont_xi.run_continuation()

    root = sol_set_xi[-1]

    cont_v_f = ContinuationHandler(num_solver, root)
    cont_v_f.add_linear_series(100, {'v_f': vf_max})
    sol_set_vf = cont_v_f.run_continuation()

    sol_set_vf.save(f'xi_first_{round(xi * 180 / 3.1415)}.bin')
