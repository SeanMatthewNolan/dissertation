...
for center in ((15, 0), (30, 0), (45, 0), (60, 0)):
    print(center)
...
for center in ((45, 15), (45, 30), (45, -15), (45, -30)):
    print(center)
    data_dir = f'star_cav_h_{center[0]:02d}_{center[1]:02d}'
    if not os.path.isdir(data_dir):
        os.makedirs(data_dir)
...
#
#     cont = ContinuationHandler(num_solver, sol_set_start[-1])
#     cont.add_linear_series(100, {'theta_f': np.deg2rad(45) * center[1] / abs(center[1])})
#     cont.add_linear_series(100, {'phi_f': np.deg2rad(center[0])})
#     cont.add_linear_series(100, {'theta_f': np.deg2rad(center[1])})
#     sol_set_center = cont.run_continuation()
#
#     for xi in np.arange(delta_xi/2, 360, delta_xi):
#         xi_rad = np.deg2rad(xi)
