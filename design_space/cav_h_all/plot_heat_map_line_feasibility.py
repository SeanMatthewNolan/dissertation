...
for sols in data:
    for sol in sols[::SKIP]:
        norms.append(np.linalg.norm(np.concatenate((sol.nu0, sol.nuf))))

cmap = plt.colormaps['viridis']
norm = LogNorm(min(norms), max(norms))
...
cbar = fig.colorbar(
        plt.cm.ScalarMappable(norm=norm, cmap=cmap), label=r'Static Adjoints Norm, $|| \boldsymbol{\nu} ||$', ax=ax)
...
ax.set_title('Magnitude of Static Adjoint Variables')
