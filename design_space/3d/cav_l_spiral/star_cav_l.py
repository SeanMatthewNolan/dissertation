...
h_size = 1000
v_size = 100
gam_size = np.deg2rad(0.1)
for n, point in enumerate(spiral_points):
    cont = ContinuationHandler(num_solver, sol_set_start[-1])
    cont.add_linear_series(n_steps, {
        'h_0': sol_set_start[-1].x[0, 0] + point[0] * h_size * n_steps,
        'v_0': sol_set_start[-1].x[3, 0] + point[1] * v_size * n_steps,
        'gamma_0': sol_set_start[-1].x[4, 0] + point[2] * gam_size * n_steps,
    })
    ray_set = cont.run_continuation()

    ray_set.save(f'./{data_dir}/ray{n:03}.bin')
