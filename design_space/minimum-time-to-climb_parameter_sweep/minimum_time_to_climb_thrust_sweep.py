...
# Look-Up Tables
thrust = thrust_table_bspline(ca.vertcat(M, h)) * k_thrust
...
if __name__ == "__main__":
    ...
    # cont = giuseppe.continuation.ContinuationHandler(num_solver, sol_set[-1])
    # cont.add_linear_series(100, {'k_thrust': 0.75})
    # sol_set = cont.run_continuation()

    cont = giuseppe.continuation.ContinuationHandler(num_solver, sol_set[-1])
    cont.add_linear_series(100, {'k_thrust': 1.5})
    sol_set_isp = cont.run_continuation()

    # Save Solution
    sol_set_isp.save('thrust_series.data')
