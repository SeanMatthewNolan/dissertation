...
ocp.add_expression('lift', 'c_l * s_ref * dyn_pres * (1 + lift_per)')
ocp.add_expression('drag', 'c_d * s_ref * dyn_pres * (1 + drag_per)')
...
ocp.add_constant('lift_per', 0)
ocp.add_constant('drag_per', 0)
...
# Perform first dimension (Drag 150 -> 50)
for drag_val in np.linspace(0.5, -0.5, 11)[1:]:
    cont = ContinuationHandler(num_solver, node_sols[-1])
    cont.add_linear_series(10, {'drag_per': drag_val})
    inter_node_set = cont.run_continuation()

    node_sols.append(inter_node_set[-1])
    inter_node_sets.append(inter_node_set)

first_dim_nodes = copy.copy(node_sols)

# Perform second dimension (Lift 150 -> 50)
for root in first_dim_nodes:
    last_node = root
    for lift_val in np.linspace(0.5, -0.5, 11)[1:]:
        cont = ContinuationHandler(num_solver, last_node)
        cont.add_linear_series(10, {'lift_per': lift_val})
        inter_node_set = cont.run_continuation()

        inter_node_sets.append(inter_node_set)

        last_node = inter_node_set[-1]
        if not np.isclose(last_node.k[22], lift_val):
            break

        node_sols.append(last_node)


out_data = {'node_sols': node_sols, 'inter_node_sets': inter_node_sets}

data_label = f'set_phi_{center[0]:03d}_theta_{center[1]:03d}'
with open(f'{data_label}.data', 'wb') as file:
