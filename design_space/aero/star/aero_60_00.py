...
center = (60, 0)
data_dir = f'set_phi_{center[0]:03d}_theta_{center[1]:03d}'
...
delta_xi = 5
target_percent = 100 / 100
for xi in np.arange(0, 360, delta_xi):
    print(xi)
    cont = ContinuationHandler(num_solver, sol_set_start[-1])
    cont.add_linear_series(200,
                           {'lift_per': target_percent * cos(np.deg2rad(xi)),
                            'drag_per': target_percent * sin(np.deg2rad(xi))},
                           )
