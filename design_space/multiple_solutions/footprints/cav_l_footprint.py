...
ocp.add_constant('v_0', 900)   # 20_000
...
guess = auto_propagate_guess(comp_prob, control=(7.5 / 180 * 3.14159, 0), t_span=100, max_step=5)
...
cont.add_linear_series_until_failure({'xi': 0.25 * np.pi / 180})
sol_set_footprint = cont.run_continuation()

sol_set_footprint.save('cav_l_footprint_left.bin')

cont = ContinuationHandler(num_solver, sol_set[-1])
cont.add_linear_series_until_failure({'xi': -0.25 * np.pi / 180})
sol_set_footprint = cont.run_continuation()

sol_set_footprint.save('cav_l_footprint_right.bin')
