...
ax_min.set_title('Secant Penalty Function')
# ax_min.set_xticks(np.array([x_min, x_mid, x_max]),
#                   (r'$C_{i, \text{min}}$', r'$(C_{i, \text{min}} + C_{i, \text{max}})/2$', r'$C_{i, \text{max}}$'))
ax_min.set_xticks(np.array([x_min, x_max]),
                  (r'$C_{i, \text{min}}$', r'$C_{i, \text{max}}$'))
...
for eps in epsilons:
    color = cmap(norm(eps))
    ax_min.plot(x, eps * utm_penalty_offset(x), color=color)
    # ax_max.plot(x, eps * rational_max(x), color=color)

ax_min.set_xlim(x_min - 0.1, x_max + 0.1)
