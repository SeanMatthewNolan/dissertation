...
ocp.add_constant('eps_beta', 1e-3)
...
ocp.add_inequality_constraint('path', 'alpha', lower_limit='-alpha_max', upper_limit='alpha_max',
                              regularizer=PenaltyConstraintHandler('eps_alpha * dyn_pres'))
ocp.add_inequality_constraint('path', 'beta', lower_limit='-beta_max', upper_limit='beta_max',
                              regularizer=PenaltyConstraintHandler('eps_beta * dyn_pres'))
...
guess = auto_propagate_guess(
        ...
        initial_states=(165_000, 0, 0, 20_000, -5 / 180 * np.pi, np.pi / 2)
)
...
cont.add_logarithmic_series(100, {'eps_beta': 1e-3})
...
sol_set_center.save('beta_ref_q.data')
