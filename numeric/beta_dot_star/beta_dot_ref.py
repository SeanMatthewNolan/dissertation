...
ocp.add_state('beta', 'beta_dot')

ocp.add_control('alpha')
ocp.add_control('beta_dot')
...
# ocp.set_cost('0', 'k_mit / 2 * (beta ** 2)', '-v**2/v_0**2')
ocp.set_cost('0', 'k_mit / 2 * (beta_dot ** 2)', '-v/v_0')
# ocp.set_cost('0', 'k_mit / 2 * atan(beta ** 2 / angle_scale**2)', '-v/v_0')
# ocp.set_cost('0', 'k_mit / 2 * (alpha ** 2 + beta ** 2)', '-v/v_0')
...
sym_prob = SymDual(ocp, control_method='differential')
sym_prob.add_post_process('compute_hamiltonian')
sym_prob.add_post_process('compute_huu')
num_solver = SciPySolver(sym_prob)

guess = auto_propagate_guess(
        sym_prob, control=(5 / 180 * 3.14159, 0.000), t_span=25,
        initial_states=(165_000, 0, 0, 20_000, -5 / 180 * np.pi, np.pi / 2, 0.)
)
...
cont.add_linear_series(20, {'phi_f': 45 / 180 * np.pi})
sol_set_start = cont.run_continuation()


step_size = np.deg2rad(0.5)
xi_rad = np.deg2rad(150)
...
sol_set_center.save('beta_dot_ref.data')
