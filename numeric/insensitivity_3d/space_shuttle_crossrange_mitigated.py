...
ocp.add_constant('a_sweep', 0)
ocp.add_constant('k', 1e-8)
...
ocp.set_cost('0', '1/2 * k * (alpha ** 2 + beta ** 2)', '-phi * (1 - a_sweep) - theta  * a_sweep')
...
with Timer('Setup Time: '):
    ...
    solver = SciPySolver(comp_dual, node_buffer=2000)
    guess = auto_propagate_guess(comp_dual, control=(10/180*3.14159, 0), t_span=100)
...
cont.add_linear_series(35, {'a_sweep': 1})
sol_set = cont.run_continuation()

cont = ContinuationHandler(solver, sol_set[-1])
cont.add_linear_series(300, {'gamma_0': 10 / 180 * 3.14159})
sol_set = cont.run_continuation()

sol_set.save('crossrange_mit.data')
