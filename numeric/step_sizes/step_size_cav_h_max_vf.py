...
# for num_step in (25, 50, 100, 250):
#     cont = ContinuationHandler(num_solver, sol_set_start[-1])
#     cont.add_linear_series(num_step, {'phi_f': 60 / 180 * np.pi}, bisection=8)
#
#     with Timer(f'{num_step}'):
#         sol_set = cont.run_continuation()
#
#     sol_set.save(f'cav_h_{num_step:d}.bin')

times = []
num_bisects = []
step_sizes = np.linspace(5, 500, 100)
for num_step in step_sizes:
    sample_times = []
    for _ in range(4):
        cont = ContinuationHandler(num_solver, sol_set_start[-1])
        cont.add_linear_series(int(num_step), {'phi_f': 60 / 180 * np.pi}, bisection=8)
...
with open('times.csv', 'w') as csv_file:
    csv.writer(csv_file).writerows(
            [step_sizes, times, num_bisects]
    )
