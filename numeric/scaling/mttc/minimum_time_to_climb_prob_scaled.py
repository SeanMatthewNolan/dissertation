...
# Scaling
t_scale = ca.MX.sym('t_scale', 1)
h_scale = ca.MX.sym('h_scale', 1)
v_scale = ca.MX.sym('v_scale', 1)
gam_scale = ca.MX.sym('gam_scale', 1)
w_scale = ca.MX.sym('w_scale', 1)

ocp.add_constant(t_scale, 1)
ocp.add_constant(h_scale, 1)
ocp.add_constant(v_scale, 1)
ocp.add_constant(gam_scale, 1)
ocp.add_constant(w_scale, 1)

# ocp.add_constraint(location='initial', expr=(t - t_0) / t_scale)
# ocp.add_constraint(location='initial', expr=(h - h_0) / h_scale)
# ocp.add_constraint(location='initial', expr=(v - v_0) / v_scale)
# ocp.add_constraint(location='initial', expr=(gam - gam_0) / gam_scale)
# ocp.add_constraint(location='initial', expr=(w - w_0) / w_scale)
#
...
#
# ocp.add_constraint(location='terminal', expr=(h - h_f) / h_scale)
# ocp.add_constraint(location='terminal', expr=(v - v_f) / v_scale)
# ocp.add_constraint(location='terminal', expr=(gam - gam_f) / gam_scale)
