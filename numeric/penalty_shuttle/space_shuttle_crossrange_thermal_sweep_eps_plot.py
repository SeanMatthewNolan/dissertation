...
fpa_sweep = gp.load_sol_set('eps_effect.data')
...
cross_f = []
cost = []
for k, sol in enumerate(fpa_sweep):
...
ax_cost.plot(epsilons, cross_f, ':', label=r'Original Cost $\bar{J}$', linewidth=2, color='C2')
ax_cost.plot(epsilons, cost, label=r'Augmented Cost $J$', linewidth=2, color='C0')
ax_cost.legend()
...
ax_cost_sens.plot(
        epsilons, np.rad2deg(pred_diff), label=r'$\int_0^{t_f} \epsilon \frac{\partial P}{\partial S} \, dt$',
        linewidth=2
)
ax_cost_sens.plot(
        epsilons, (cross_f - np.asarray(cost)), '-.',
...
zoom_slice = slice(-60, -1)
ax_cost_sens_zoom.plot(
        epsilons[zoom_slice], np.rad2deg(pred_diff)[zoom_slice],
        label=r'$\int_0^{t_f} \epsilon \frac{\partial P}{\partial S} \, dt$',
        linewidth=2
)
ax_cost_sens_zoom.plot(
        epsilons[zoom_slice], (cross_f - np.asarray(cost))[zoom_slice], '-.',
        linewidth=2, label='Penalty'
)

ax_cost_sens_zoom.legend()
ax_cost_sens_zoom.set_xscale('log')
